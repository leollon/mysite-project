...
def validate_view(request, token):
    ...
    """
    if not request.user.is_valid and request.user.valid_account(
        token.encode(encoding="ascii")
    ):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if user is None:
                ...
                msg = {
                    "invalidation": "This account was banned. It can't not be used to login this site."
                }
...
def password_reset_request(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            msg = {"notification": "The email including token has sent to you."}
            return render(request, "user/send_ok.html", msg)
        else:
            return render(
                request, "user/password_reset_request.html", {"form": form}
            )
    else:
